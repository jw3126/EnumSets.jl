...
@testset "exhaustive enums" begin
    ...
    @test sizeof(enumsettype(X1)) == 1
    @test length(instances(X1)) == 8
    fuzz_booleans(enumsettype(X1))

    @test sizeof(enumsettype(X2)) == 2
    @test length(instances(X2)) == 2*8
    fuzz_booleans(enumsettype(X2))

    @test sizeof(enumsettype(X4)) == 4
    @test length(instances(X4)) == 4*8
    fuzz_booleans(enumsettype(X4))

    @test sizeof(enumsettype(X8)) == 8
    @test length(instances(X8)) == 8*8
    fuzz_booleans(enumsettype(X8))

    @test sizeof(enumsettype(X16)) == 16
    @test length(instances(X16)) == 16*8
    fuzz_booleans(enumsettype(X16))
end
